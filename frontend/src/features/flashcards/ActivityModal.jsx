		...
		<Modal open onClose={onClose}>
			...
			<div className="mt-4 flex gap-3">
				{[
					...
					<div key={s.label} className="flex-1 rounded-2xl bg-bg-secondary px-4 py-3 text-center">
			...
			<div className="mt-5">
				{loading ? (
					<div className="flex h-32 items-center justify-center text-sm text-body">Loading…</div>
				) : (
					<>
						<div className="flex items-end gap-[3px] h-32">
							{data.map((d, i) => {
								const height = d.count === 0 ? 2 : Math.max(8, (d.count / max) * 112);
									...
									>
										...
										<div className="absolute bottom-full mb-1.5 left-1/2 -translate-x-1/2 hidden group-hover:flex flex-col items-center z-10 pointer-events-none">
											<div className="rounded-lg bg-title px-2 py-1 text-[10px] text-bg whitespace-nowrap">
												{d.count} card{d.count !== 1 ? "s" : ""}<br />
												<span className="opacity-60">{MONTH_LABELS[date.getMonth()]} {date.getDate()}</span>
											</div>
							...
							{data.map((d, i) => {
								...
								return (
									<div key={i} className="flex-1 text-center">
										{show && (
											<span className={`text-[9px] leading-tight ${d.isToday ? "font-bold text-folder-blue" : "text-body"}`}>
												{date.getDate()}<br />{MONTH_LABELS[date.getMonth()]}
											</span>
										)}
									</div>
								);
