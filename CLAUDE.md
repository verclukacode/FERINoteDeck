...
- **Note editor**: `features/notes/editor/` — a block-based editor (h1/h2/text/bullet/numbered/checklist/image/separator). A note's body is stored as one markdown string in `Page.content` (wrapped in a `<<<NoteDeckMD>>>` sentinel); `editor/markdown.js` parses/serializes it; no per-block storage. See `frontend/docs/editor.md`.
