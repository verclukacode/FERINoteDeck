...
- **Backend** (`backend/src/index.ts`) — Express, persists to **MySQL via Prisma** (`backend/prisma/schema.prisma`, client singleton `src/lib/prisma.ts`). Routers (all behind `requireAuth`):
  ...
  - `POST /api/pages/:id/generate-deck` (on `pages.ts`) — AI flashcard generation
    from a note's content; tier-gated like `/api/import`. Returns `{title, cards}` —
    not persisted; the frontend confirms a target folder before save.
  - `POST /api/pages/:id/chat` (on `pages.ts`) — chat with OpenAI grounded in the
    note's content. Tier-gated. Body is `{messages: [{role, content}]}` (≤ 40 turns,
    ≤ 4000 chars each). Streams the reply as raw `text/plain` chunks (no SSE wrapper)
    via `chatAboutNoteStream` in `src/lib/openai.ts`.
...
- **Note editor**: `features/notes/editor/` — a block-based editor (h1/h2/text/bullet/numbered/checklist/image/separator). A note's body is one markdown string in `Page.content` (`<<<NoteDeckMD>>>` sentinel); `editor/markdown.js` parses/serializes it. The editor **does not auto-save** — `NotePanel` has a Save button that calls `updatePageContent`. Images upload to `/api/images` and are referenced by URL. **LaTeX**: inline `$x^2$` and display `$$\int x\,dx$$` (also `\(...\)` / `\[...\]`) are rendered via KaTeX as atomic `contenteditable=false` spans carrying `data-tex` for round-trip in `editor/inlineFormat.js` — math renders on blur, edit by deleting the rendered span. The same renderer feeds `NoteReadView`, `MarketplacePreview` flashcards, and the study session `PromptCard`. See `frontend/docs/editor.md`.
- **Note chat assistant**: `features/notes/NoteChatButton.jsx` + `NoteChatPanel.jsx` — a floating Duolingo-style purple chat button at the bottom-right of `NotePanel` that opens a 420 px-wide side panel beside the editor (flex sibling, not an overlay). Posts to `/api/pages/:id/chat` and streams the reply as raw `text/plain` chunks. Chat state (`chatOpen`, `chatMessages`) lives in `NotePanel`; history is persisted per-page in `sessionStorage` under `notedeck-chat:<pageId>` (last 30 messages, content clipped to 4000 chars). Tier-gated: basic accounts see the locked button.
- **Flashcards (SRS)**: `features/flashcards/` — folder → deck → card, persisted in MySQL (`FlashcardFolder`/`Deck`/`FlashCard` + a `Review` revlog + per-user `StudySettings`). Studying uses an **Anki-style SM-2** scheduler (`backend/src/lib/srs.ts`); all scheduling times are **unix ms (`BigInt`)**, durations seconds, ease permille — responses pass through `backend/src/lib/serialize.ts`. `StudySession` loads `GET /api/decks/:id/queue` (daily limits) and persists each answer via `POST /api/cards/:id/answer`, then calls `FlashcardsContext.mergeCards([updated])` so the deck panel's `CardStateBar` and queue counts reflect the new SRS state immediately (whether the session is finished or closed mid-way). State lives in `FlashcardsContext`, which also owns **`pendingDeckInvites`** + accept/decline. See `frontend/docs/flashcards.md`.
